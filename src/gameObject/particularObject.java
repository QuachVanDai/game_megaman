...
public abstract class particularObject extends gameObject {
	...
	public boolean isObjectOutOfCameraView() {
		...
		if (getPosX() - getG_world().camera.getPosX() > getG_world().camera.getWidthView()
				|| getPosX() - getG_world().camera.getPosX() < -50
				|| getPosY() - getG_world().camera.getPosY() > getG_world().camera.getHeightView()
				|| getPosY() - getG_world().camera.getPosY() < -50)
			return true;
		else
			return false;
...
//				}
	...
	public void drawBoundForCollisionWithMap(Graphics2D g2) {
        ...
        g2.drawRect(rect.x - (int) getG_world().camera.getPosX(), rect.y - (int) getG_world().camera.getPosY(), rect.width, rect.height);
